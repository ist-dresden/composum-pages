...
<cpp:element var="row" type="com.composum.pages.components.model.container.Row" cssAdd="row">
    ...
    <cpn:div test="${not empty row.title}" class="${rowCSS}_header">
        <cpn:text tagName="h2" value=" ${row.title}" i18n="true" class="${rowCSS}_title"/>
    </cpn:div>
    <c:forEach items="${row.columns}" var="column" varStatus="loop">
        <div class="${rowCSS}_column ${rowCSS}_column-${loop.first?'first':loop.last?'last':loop.index} col ${column}">
